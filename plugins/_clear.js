...
const paths = {
  HarukaJadiBot: `./${jadi}/`,
  HarukaSession: `./${sessions}/`
};

function cleanSubbotDirectories() {
  for (const [name, path] of Object.entries(paths)) {
    if (name === 'HarukaSession') continue; // Skip SanSession for this function
...
function cleanHarukaSession() {
  const sessionPath = paths.HarukaSession;
...
setInterval(cleanHarukaSession, 60 * 1000);
